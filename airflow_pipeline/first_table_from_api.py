...
def get_patients():
    resp = requests.get('http://10.32.22.6:56733/patients')
    patients = resp.json()['json_patients']
    return patients

def combine_notes_and_admissions_and_codes(admissions, all_notes, icd_codes, patients):
    ...
    for admission in admissions:
        ...
        #add demographics from MIMIC Patients table
        patient_id = admission['patient_id']
        patient = patients_df.query('patient_id=='+str(patient_id))
        patient = patient.iloc[0]
        admission['gender'] = patient['gender']
        admission['dob'] = patient['dob']
    return admissions
