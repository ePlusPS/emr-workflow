...
def make_input():
    _, top_n_feat_df_json_encoded, _ = xgb_read_from_db('feat_xgb_los')
    top_n_feat_df = pd.read_json(top_n_feat_df_json_encoded.decode())

    _, top_n_neg_feat_df_json_encoded, _ = xgb_read_from_db('neg_feat_xgb_los')
    top_n_neg_feat_df = pd.read_json(top_n_neg_feat_df.decode())

    _, top_n_med_df_json_encoded, _ = xgb_read_from_db('med_xgb_los')
    top_n_med_df = pd.read_json(top_n_med_df_json_encoded.decode())

    _, top_n_neg_med_df_json_encoded, _ = xgb_read_from_db('neg_med_xgb_los')
    top_n_neg_med_df = pd.read_json(top_n_neg_med_df_json_encoded.decode())

    _, top_n_lda_df_json_encoded, _ = xgb_read_from_db('lda_xgb_los')
    top_n_lda_df = pd.read_json(top_n_lda_df_json_encoded.decode())

    _, top_n_demo_df_json_encoded, _ = xgb_read_from_db('demo_xgb_los')
    ...
    standard_write_to_db('new_records_los_tf_input', tf_input_json_encoded)
