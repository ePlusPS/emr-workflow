...
class SomeCallableTest(unittest.TestCase):
    ...
    def test_make_one_hot_standard(self):
        ...
        one_hot = xgb_readmission_lda_topics.make_one_hot(df)
    ...
    def test_train_xgb_model_not_null(self):
        df = pd.DataFrame()
        row1 = {'admission_id':1, 'lda_ngrams':['blood_pressure_greater_than_100','fractured_rib_and_broken_femur'], 'readmission': 1}
        row2 = {'admission_id':2, 'lda_ngrams':[], 'readmission': 0}
        row3 = {'admission_id':3, 'lda_ngrams':['fractured_rib_and_broken_femur'], 'readmission': 0}
        df = df.append(row1, ignore_index=True)
        df = df.append(row2, ignore_index=True)
        df = df.append(row3, ignore_index=True)
        ...
        xgb_model = xgb_readmission_lda_topics.train_xgb_model(df, one_hot)
    ...
    def test_add_predictions_column_no_null_entries(self):
        ...
        updated_df = xgb_readmission_lda_topics.add_predictions_column(df, xgb_model, one_hot)
    ...
    def test_make_top_n_features_check_num_features(self):
        df = pd.DataFrame()
        row1 = {'admission_id':1, 'lda_ngrams':['blood_pressure_greater_than_100','fractured_rib_and_broken_femur'], 'readmission': 1}
        row2 = {'admission_id':2, 'lda_ngrams':['headache', 'nausea'], 'readmission': 0}
        row3 = {'admission_id':3, 'lda_ngrams':['fractured_rib_and_broken_femur'], 'readmission': 0}
        df = df.append(row1, ignore_index=True)
        df = df.append(row2, ignore_index=True)
        df = df.append(row3, ignore_index=True)
    ...
    def test_make_top_n_features_check_num_features(self):
        ...
        xgb_model = xgb_readmission_lda_topics.train_xgb_model(df, one_hot)
        top_2_df = xgb_readmission_lda_topics.make_top_n_features(xgb_model, one_hot, 2)
