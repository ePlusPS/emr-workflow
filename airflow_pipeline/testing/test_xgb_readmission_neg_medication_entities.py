...
class SomeCallableTest(unittest.TestCase):
    ...
    def test_make_one_hot_standard_true_case(self):
        ...
        one_hot = xgb_readmission_neg_medication_entities.make_one_hot(df)
        # print (one_hot)
    ...
    def test_train_xgb_model_not_null(self):
        df = pd.DataFrame()
        row1 = {'admission_id':1, 'neg_medication_entities':['no_electrolytes','no_insulin', 'no_lasix', 'not_tpn', 'no_kayexalate_name8', 'no_plan', 'no_alcohol'], 'readmission': 5}
        row2 = {'admission_id':2, 'neg_medication_entities':[], 'readmission': 8}
        row3 = {'admission_id':3, 'neg_medication_entities':['no_insulin', 'no_alcohol'], 'readmission': 9}
        df = df.append(row1, ignore_index=True)
        df = df.append(row2, ignore_index=True)
        df = df.append(row3, ignore_index=True)
        ...
        xgb_model = xgb_readmission_neg_medication_entities.train_xgb_model(df, one_hot)
        # print (one_hot)
    ...
    def test_add_predictions_column_no_null_entries(self):
        ...
        updated_df = xgb_readmission_neg_medication_entities.add_predictions_column(df, xgb_model, one_hot)
        # print (updated_df)
    ...
    def test_make_top_n_features_check_num_features(self):
        ...
        xgb_model = xgb_readmission_neg_medication_entities.train_xgb_model(df, one_hot)
        top_2_df = xgb_readmission_neg_medication_entities.make_top_n_features(xgb_model, one_hot, 2)
        # print (top_2_df)
