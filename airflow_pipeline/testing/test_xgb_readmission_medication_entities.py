...
class SomeCallableTest(unittest.TestCase):
    ...
    def test_make_one_hot_standard_true_case(self):
        ...
        one_hot = xgb_readmission_medication_entities.make_one_hot(df)
        # print (one_hot)
    ...
    def test_make_one_hot_standard_false_case(self):
        df = pd.DataFrame()
        row1 = {'admission_id':1, 'medication_entities':['morphine', 'famotidine', 'plan', 'hct', 'cap', 'today'], 'readmission': 1}
        row2 = {'admission_id':2, 'medication_entities':[], 'readmission': 0}
        row3 = {'admission_id':3, 'medication_entities':['hct'], 'readmission': 0}
        df = df.append(row1, ignore_index=True)
        df = df.append(row2, ignore_index=True)
        df = df.append(row3, ignore_index=True)
    ...
    def test_train_xgb_model_not_null(self):
        ...
        xgb_model = xgb_readmission_medication_entities.train_xgb_model(df, one_hot)
        if (one_hot.empty):
            True
        if (one_hot.all):
            True
    ...
    def test_add_predictions_column_no_null_entries(self):
        df = pd.DataFrame()
        row1 = {'admission_id':1, 'medication_entities':['morphine','famotidine', 'plan', 'hct', 'cap', 'today'], 'readmission': 0}
        row2 = {'admission_id':2, 'medication_entities':[], 'readmission': 0}
        row3 = {'admission_id':3, 'medication_entities':['hct'], 'readmission': 0}
        df = df.append(row1, ignore_index=True)
        df = df.append(row2, ignore_index=True)
        ...
        xgb_model = xgb_readmission_medication_entities.train_xgb_model(df, one_hot)
        """
        updated_df = xgb_readmission_medication_entities.add_predictions_column(df, xgb_model, one_hot)
        null_count = 0
    ...
    def test_make_top_n_features_check_num_features(self):
        ...
        xgb_model = xgb_readmission_medication_entities.train_xgb_model(df, one_hot)
        top_2_df = xgb_readmission_medication_entities.make_top_n_features(xgb_model, one_hot, 2)
        # print (top_2_df)
