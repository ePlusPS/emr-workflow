...
class SomeCallableTest(unittest.TestCase):
    ...
    def test_xgb_readmission_demographics_make_one_hot_standard_gender_check(self):
        df = pd.DataFrame()
        ...
        # self.df = xgb_readmission_demographics.make_one_hot(df)
        # print (self.df)
    ...
    def test_male_gender_check(self):
        ...
        df = df.append(row, ignore_index=True)
        # updated_df = xgb_readmission_demographics.make_one_hot(df)
    ...
    def test_xgb_readmission_demographics_train_xgb_model_not_null(self):
        df = pd.DataFrame()
        ...
        xgb_model = xgb_readmission_demographics.train_xgb_model(df, one_hot)
        # print (one_hot)
    ...
    def test_xgb_readmission_demographics_add_predictions_column_no_null_entries(self):
        df = pd.DataFrame()
        row1 = {'admission_id':1, 'feature_entities':['accident','not_well_seen'], 'readmission': 1}
        row2 = {'admission_id':2, 'feature_entities':[], 'readmission': 0}
        row3 = {'admission_id':3, 'feature_entities':['not_well_seen'], 'readmission': 0}
        ...
        # updated_df = xgb_readmission_demographics.add_predictions_column(df, xgb_model, one_hot)
        # print (updated_df)
    ...
    def test_xgb_readmission_demographics_make_top_n_features_check_num_features(self):
        df = pd.DataFrame()
        row1 = {'admission_id':1, 'feature_entities':['accident','not_well_seen'], 'readmission': 1}
        ...
        xgb_model = xgb_readmission_demographics.train_xgb_model(df, one_hot)
        top_2_df = xgb_readmission_demographics.make_top_n_features(xgb_model, one_hot, 2)
        # print (top_2_df)
    ...
    def test_xgb_readmission_demographics_make_top_n_features_readmission_check(self):
        df = pd.DataFrame()
        ...
        if one_hot['readmission'].to_list() == False:
            print ("** Test for readmission check - No readmission **")
        if one_hot['readmission'].to_list() == True:
            print ("** Test for readmission check - readmission found **")
