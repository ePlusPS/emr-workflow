...
def make_tf_predictions():
    tf_input_df_json_encoded = standard_read_from_db('new_records_los_tf_input')
    tf_input_df = pd.read_json(tf_input_df_json_encoded.decode())
    
    model = keras.models.load_model('tf_los/most_recent')

    predictions = predict_with_model(tf_input_df, model)
    tf_input_df['tf_los_pred'] = predictions

    tf_input_json_encoded = tf_input_df.to_json().encode()
    standard_write_to_db('new_records_los_tf_predictions', tf_input_json_encoded)
